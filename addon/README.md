...
The addon key is `pl-favat-v3`. GOBL recognises it as an approved external
addon, but the implementation lives here, so the module must be imported for
`Calculate` and `Validate` to succeed:
